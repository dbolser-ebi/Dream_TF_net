...
class MultiConvNet():
    def __init__(self, model_dir, batch_size=256, num_channels=4, num_epochs=2,
                 sequence_width=200, num_tfs=1, eval_size=0.2, early_stopping=100,
        ...
        self.tf_dnase_features = tf.placeholder(tf.float32, shape=(batch_size, num_dnase_features),
                                                name='dnase_features')
        ...
        self.tf_train_labels = tf.placeholder(tf.float16, shape=(batch_size, num_tfs), name='labels')
        ...
        with tf.variable_scope(self.name) as main_scope:
            ...
            with tf.variable_scope('MERGE') as scope:
                ...
                with tf.variable_scope('output') as outscope:
                    logits = fully_connected(drop, self.num_tfs, None)
                ...
                for epoch in xrange(1, self.num_epochs + 1):
                    ...
                    for offset in xrange(0, num_examples - self.batch_size, self.batch_size):
                        ...
                        for celltype_idx in xrange(y.shape[1]):
                            ...
                            batch_shapes = np.reshape(S_train[offset:end, :],
                                                      (self.batch_size, self.height, self.sequence_width,
                                                       self.num_shape_features))
                            batch_labels = np.reshape(y_train[offset:end, celltype_idx], (self.batch_size, 1))
                            batch_dnase_features = np.reshape(
                                da_train[offset:end, :self.num_dnase_features, celltype_idx],
                    ...
                    for offset in xrange(0, num_examples - self.batch_size, self.batch_size):
                        ...
                        for celltype_idx in xrange(y.shape[1]):
                            ...
                            batch_dnase_features = np.reshape(
                                da_val[offset:end, :self.num_dnase_features, celltype_idx],
                                (self.batch_size, self.num_dnase_features))
                            feed_dict = {self.tf_sequence: batch_sequence,
                                         self.tf_shape: batch_shapes,
                                         self.tf_train_labels: batch_labels,
                            ...
                            accuracies.append(
                                100.0 * np.sum(np.abs(prediction - batch_labels) < 0.5) / batch_labels.size)
